...
describe('LocalAPIService vertical slice', () => {
  ...
  it('migrates a version-two persona without discarding identity state', async () => {
    const legacy = structuredClone(PERSONA_SEEDS.rajesh) as unknown as {
      ...
      epfo: { kyc?: unknown; passbook?: unknown; nomination?: unknown; employment: Array<Record<string, unknown>> }
    }
    legacy.schemaVersion = 2
    delete legacy.epfo.kyc; delete legacy.epfo.passbook; delete legacy.epfo.nomination
    legacy.epfo.employment = legacy.epfo.employment.map((record) => {
      const copy = { ...record }
      delete copy.memberId; delete copy.current; delete copy.balance
      return copy
    })
    localStorage.setItem('nagrik:persona:rajesh:state:v2', JSON.stringify(legacy))
  ...
  it('blocks an unready claim and prevents duplicate submission', async () => {
    ...
    expect((await service.getActivity('rajesh')).filter((event) => event.title === 'activity.events.claimReceived')).toHaveLength(1)
  })
  ...
  it('preserves cached passbook data when refresh fails', async () => {
    ...
    await expect(service.refreshPassbook('rajesh')).rejects.toThrow('SERVICE_UNAVAILABLE')
    expect(await service.getPassbook('rajesh')).toEqual(cached)
    expect((await service.refreshPassbook('rajesh')).employers).toEqual(cached.employers)
  })

  it('revalidates service dates and submits one transfer', async () => {
    await service.resolveMismatch({ personaId: 'rajesh', mismatchId: 'mismatch-name', canonicalValue: 'Rajesh Kumar' })
    expect((await service.validateTransfer('rajesh', 'emp-r-1', 'emp-r-2')).ready).toBe(false)
    await service.updateEmploymentExit('rajesh', 'emp-r-1', '2025-05-14')
    expect((await service.validateTransfer('rajesh', 'emp-r-1', 'emp-r-2')).ready).toBe(true)
    const input = { sourceEmploymentId: 'emp-r-1', destinationEmploymentId: 'emp-r-2', declarationAccepted: true, otp: '123456' }
    const first = await service.submitTransfer('rajesh', input)
    const duplicate = await service.submitTransfer('rajesh', input)
    expect(duplicate.reference).toBe(first.reference)
    expect((await service.getActivity('rajesh')).filter((event) => event.title === 'activity.events.transferReceived')).toHaveLength(1)
  })

  it('autosaves and verifies an exact nomination allocation', async () => {
    const nominees = [
      { id: 'n-1', name: 'Meera Kumar', relationship: 'SPOUSE' as const, dateOfBirth: '1988-04-12', share: 60 },
      { id: 'n-2', name: 'Asha Kumar', relationship: 'PARENT' as const, dateOfBirth: '1962-01-20', share: 40 },
    ]
    expect((await service.saveNominationDraft('rajesh', nominees)).status).toBe('DRAFT')
    await expect(service.submitNomination('rajesh', nominees.map((item, index) => ({ ...item, share: index === 0 ? 50 : 40 })), '123456')).rejects.toThrow('NOMINATION_ALLOCATION_INVALID')
