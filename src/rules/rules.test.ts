...
describe('persona contracts', () => {
  it('parses both personas through the same Zod schema', () => {
    expect(personaSeedSchema.safeParse(PERSONA_SEEDS.ananya).success).toBe(
      true,
    );
    expect(personaSeedSchema.safeParse(PERSONA_SEEDS.rajesh).success).toBe(
      true,
    );
    expect(Object.keys(PERSONA_SEEDS.ananya).sort()).toEqual(
      Object.keys(PERSONA_SEEDS.rajesh).sort(),
    );
  });
...
describe('identity and PF rules', () => {
  ...
  it('returns every named claim rule independently', () => {
    ...
    expect(
      validation.results.find((result) => result.code === 'NAME_MATCH')?.passed,
    ).toBe(false);
  });
  ...
  it('detects overlapping transfer service dates', () => {
    ...
    expect(validation.ready).toBe(false);
    expect(
      validation.results.find((rule) => rule.code === 'DATE_OVERLAP')?.passed,
    ).toBe(false);
  });

  it('requires nominee shares to total exactly 100%', () => {
    expect(validateNomineeAllocation([50, 50])).toEqual({
      total: 100,
      valid: true,
    });
    expect(validateNomineeAllocation([60, 30])).toEqual({
      total: 90,
      valid: false,
    });
