...
| 12. Localization                             | **Partial** | i18next, English/Hindi/Bengali resources, language persistence, locale-aware dates/currency, and flexible layouts are implemented. All inline English copy in shared components and feature pages (home, activity, identity, EPFO, auth, profile, shell, error boundary) now routes through translation keys, including previously English-only shared-component defaults (back links, step progress, detail-sheet close). Every rule-explanation and status string in the tax, EPFO, and activity resource files has complete, distinct Hindi and Bengali translations with full key parity across all three languages (verified programmatically); a manual visual review across all three languages confirmed no residual English text on the core citizen journeys.                                                                                                                                                                                                                                                                                 | Perform visual review at the standard 390/768/1024/1440 px breakpoints (tracked under Section 14) and manual accessibility verification in each language.                                                                                                                                                                    |
| 13. Accessibility                            | **Partial** | Semantic landmarks/headings, labelled controls, status text with icons, comparison semantics, live regions, contrast, reflow, reduced motion, and automated axe smoke coverage are implemented. A skip-to-main-content link and route-focus handling (focus moves to `<main>`, scroll resets) now cover every route. A reusable itemized `ErrorSummary` component exists and is wired into the nomination form's multi-field details step. Masked identity, EPFO KYC, UAN, and tax bank/refund values carry accessible names (e.g. "Bank account ending in 4210") across the identity comparison table/cards, profile, EPFO records, and tax pages.                                                                                                                                                                                                                                                                                                                                                                                                     | Apply the `ErrorSummary` pattern to remaining multi-field forms as they arise, and perform manual keyboard, screen-reader, and 200–400% zoom verification.                                                                                                                                                                   |
...
- [x] Complete every submitted-path string and rule explanation in Hindi and Bengali.
- [x] Finish route focus, skip navigation, error summaries, and accessible masked values.
- [ ] Perform manual keyboard/screen-reader/zoom verification.
